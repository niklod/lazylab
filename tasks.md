...
- [ ] Global close/merge shortcuts available from any detail pane (Overview, Diff, Conversation, Pipeline, Log) — not only from the MRs list
  - **Rationale:** `x`/`M` are currently bound only on `ViewMRs` (see `views.go` bind table in G5 outcome). When the user is inside a detail tab (typical flow: select MR → tab into Pipeline/Overview to review → hit `M`), the keypress is swallowed by the focused pane with no effect, forcing them to `h` back to the MRs pane first.
  - **DoD:** pressing `x` or `M` from `ViewDetail` (Overview), `ViewDetailDiffTree`, `ViewDetailDiffContent`, `ViewDetailConversation`, `ViewDetailPipelineStages`, `ViewDetailPipelineLog` opens the close/merge modal for the currently selected MR (`DetailView.CurrentMR()` or equivalent getter). Keys remain ignored on `ViewRepos` (no MR context) and on the modal itself (already trapped by FocusOrder override). State guard + transient status behaviour matches existing MRs-pane path: non-opened MRs show `Cannot close/merge: MR !N is <state>` (routed to `MRsView.SetTransientStatus` so toast appears consistently). Post-action refresh keeps working: `MRs.SetProject` + `Detail.SetMR` chain runs regardless of origin pane. Modal focus trap already handles Enter/Esc/d/s — no changes needed there.
  - **Implementation hint:** add a single shared `openCloseModal`/`openMergeModal` helper in `internal/tui/views/mr_actions.go` that pulls the MR from `DetailView` when `MRsView` cursor is out of sync, then register the `x` and `M` bindings across the detail-family view names in `internal/tui/views.go` — same pattern as the existing `[`/`]` tab-cycle bindings which are duplicated per detail view (G4 diff sub-task documents the rationale: gocui dispatches by focused view name).
  - **Testing:** unit — extend `mr_actions_test.go` to assert the helper resolves MR from `DetailView` when invoked from detail panes; table-test the keybind registration covers all 6 detail view names. E2E — extend `tests/e2e/mr_actions_test.go`: after selecting MR, `]` into Pipeline tab, press `M`, assert modal opens with correct MR title + branches; same for `x` from Overview and from Conversation; verify non-open MR from detail pane surfaces `Cannot merge` toast without opening modal.
